...
var scene_config = {
    ...
    tileSize : 40, //Alterar aqui o tamanho da cena
...
function graphics_end() {
    graphics.game.scene.scenes[0].drawGameOver()
...
class GraphicsManager {
    constructor(game_config, scene_config, graphics_config) {
        ...
        game_config.userid = start_info.userid   
        console.log("graphics print")
        console.log(game_config.userid)
...
class OvercookedScene extends Phaser.Scene {
    ...
    set_state(state) {
        ...
        this.hud_data.time = state.time_left.toFixed(1)  //Hack to round to decimals
        this.hud_data.time_ball= state.time_ball.toFixed(1) //Hack
        
        this.slip = state.slip
    ...
    preload() {
        ...
        this.load.atlas("slip",
            this.assets_loc + "slip.png",
            this.assets_loc + "terrain.json");
        
        this.load.atlas("chefs",
            this.assets_loc + "chefs.png",
            this.assets_loc + "chefs.json");
        this.load.atlas("astros",
            this.assets_loc + "astros.png",
            this.assets_loc + "chefs.json");
    ...
    update() {
        ...
        if (typeof(this.hud_data) !== 'undefined') {
            ...
            this._drawHUD(this.hud_data, this.sprites, board_height, width);
        }
    }
    drawGameOver() {
        ...
        if (typeof(this.hud_data.score) !== 'undefined') {  
            let score = 100 - this.hud_data.time - Math.round(this.hud_data.time_ball)
            score = score.toFixed(2) // show 2 decimals
            this._drawScore(score, this.sprites, board_height, width);
        }
    }
    drawLevel() {
        // Fill canvas with white
    ...
    _drawState (state, sprites) {
        ...
        //draw environment to redo ice tiles
        if(this.prev_ice_slip_x !== -1) { //if there was a slip last time
            let tile = this.add.sprite( //Change floor
                        this.prev_ice_slip_x,
                        this.prev_ice_slip_y,
                        "tiles",
                        'ice.png'
                    );
            tile.setDisplaySize(this.tileSize, this.tileSize);
            tile.setOrigin(0);
            this.prev_ice_slip_x = -1 //reset flag

        } 

        //clean any split text
        if (typeof(sprites['slip']) !== 'undefined') {
            sprites['slip'].setText(" ");
        }
        ...
        for (let pi = 0; pi < state.players.length; pi++) {
            ...
            if (typeof(sprites['chefs'][pi]) === 'undefined') {
                ...
                if (this.player_colors[pi] == 'green') {
                    if (this.terrain[y][x] == 'I' && this.slip == true ){ 
                        let tile = this.add.sprite( //Change floor
                                    this.tileSize * x,
                                    this.tileSize * y,
                                    "slip",
                                ...
                                tile.setDisplaySize(this.tileSize, this.tileSize);
                                tile.setOrigin(0);
                                this.prev_ice_slip_x = this.tileSize * x
                                this.prev_ice_slip_y = this.tileSize * y

                        let slip_text = "???";
                        let splip_x = x*this.tileSize
                        let splip_y = y*this.tileSize-40
                        sprites['slip'] = this.add.text(
                                splip_x, splip_y, slip_text,
                            {
                                font: "30px Arial",
                                fill: "black",
                                align: "left"
    ...
    _drawHUD(hud_data, sprites, board_height, width) {
        ...
        if (typeof(hud_data.time_ball) !== 'undefined') {
            this._drawTimeBall(hud_data.time_ball, sprites, board_height, width); 
        } 
    ...
    _drawScore(score, sprites, board_height, width) { 
        score = "Score: "+score;
        if (typeof(sprites['score']) !== 'undefined') {
            ...
            sprites['score'] = this.add.text(
                width/2 - width/4, board_height/2, score, 
                {
                    font: "50px Arial",
                    fill: "navy",
                    align: "center"
    ...
    _drawTimeLeft(time_left, sprites, board_height) { //Changed to display the game time
        time_left = "Game Time: "+time_left;
        if (typeof(sprites['time_left']) !== 'undefined') {
            sprites['time_left'].setText(time_left);
        }
        else {
            sprites['time_left'] = this.add.text(
                5, board_height + 15, time_left,
                {
                    font: "27px Arial",
                    fill: 	"goldenrod",
                    align: "left"
    ...
    _drawTimeBall(time_ball, sprites, board_height, width) { //Changed to display the game time
        time_ball = "Time with ball: "+time_ball;
        if (typeof(sprites['time_ball']) !== 'undefined') {
            sprites['time_ball'].setText(time_ball); 
        }
        else {
            
            sprites['time_ball'] = this.add.text( 
                325, board_height + 15, time_ball,
                {
                    font: "27px Arial",
                    fill: "goldenrod",
                    align: "right"
