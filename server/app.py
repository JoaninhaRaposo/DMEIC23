...
import os
import uuid
#Select which mode you are playing
MODE = "human_player_bluff"
#MODE = "high_real_ai_agent"
#MODE = "low_real_ai_agent"
...
def index():
    ...
    #unique_number = datetime.datetime.now() + math.random()
    return render_template('index.html', agent_names=agent_names, unique_number=unique_number, layouts=LAYOUTS,)
...
def play_game(game, fps=30):
    ...
    while status != Game.Status.DONE and status != Game.Status.INACTIVE:
        ...
        if layout == "level_one":
            t = jsonp['state']['state']['timestep']
            state = jsonp['state']['state']
            time_ball = jsonp['state']['time_ball']
            game_time = jsonp['state']['time_left']
            timestep_log = LogFrame(t, state, time_ball, game_time)
            log.append(timestep_log)
            user_id = jsonp['userid']
            level = "lvl1"

        elif layout == "level_two":
            t = jsonp['state']['state']['timestep']
            state = jsonp['state']['state']
            time_ball = jsonp['state']['time_ball']
            game_time = jsonp['state']['time_left']
    ...
    if layout == "level_one" or layout == "level_two":
        
        directory = f"logfiles/{MODE}/{level}"

        if not os.path.exists(directory):
            os.makedirs(directory)
        
        log_file = f"logfiles/{MODE}/{level}/logfile_{user_id}_{level}.pickle"
...
if __name__ == '__main__':
    ...
    host = os.getenv('HOST', '146.193.226.27') #Use this host for docker server
    port = int(os.getenv('PORT', 80))  #Use this port for docker server

    #host = os.getenv('HOST','0.0.0.0') #Use this host for DLAI server
    #port = int(os.getenv('PORT', 81))  #Use this port for DLAI server
